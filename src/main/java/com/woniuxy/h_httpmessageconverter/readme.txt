...
2. 客户端想要什么格式的数据，服务器未必就会给什么格式的数据，因为这只是一个http规范而已，
...
5. SpringMVC中HttpMessageConverter的工作原理：
    ...
    d. SpringMVC框架读取出请求头中的content-type的值，比如为application/x-www-form-urlencoded（也就是客户端告诉服务器我给你发的是"键值对"格式的数据）
       SpringMVC根据获取到的content-type的值，去找出所有与此content-type的值匹配的HttpMessageConverter对象。
       
        如果一个都找不到，则抛出异常：HttpMediaTypeNotSupportedException （当springmvc默认没有创建StringHttpMessageConverter的时候，就有这种可能）
                        ...
                        <bean class="com.fasterxml.jackson.databind.ObjectMapper">
                            <property name="dateFormat">
                                <bean class="java.text.SimpleDateFormat">
                                	<!-- MappingJackson2HttpMessageConverter要求的默认的日期格式是yyyy-MM-dd，如果要修改则添加以下配置： -->
